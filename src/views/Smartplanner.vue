    ...
    <div class="nabnee">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-8 text-center">
                            <h1 v-if="lang=='english'" class="display-3">Smart Planner
                                <span>Plan for your dream home</span>
                            </h1>
                            <h1 v-if="lang=='arabic'" class="display-3 ">المُخَطِّط الذكي
                                <span>خَطِّط ماليا لبناء بيتك. احصل على عروض أسعار</span>
                            </h1>
                            <router-link to="/plans"><span v-if="lang=='english'">Saved Plans</span><span v-if="lang=='arabic'">عرض ملفات الأسعار المحفوظة</span> <i class="fa fa-chevron-right"></i></router-link>
                        </div>
                    </div>
                </div>
            </div>
          ...
          <div class="row justify-content-center">
            ...
            <div class="col-md-3 col-4" id="initialdiv1">
              <base-button
                ...
                block
              >
                <span v-if="lang=='english'">Start <span class="d-none d-md-inline">Planning</span></span><span v-if="lang=='arabic'">ابدء <span class="d-none d-md-inline">التخطيط</span></span>
              </base-button>
            </div>
                      ...
                      <div
                              ...
                              <base-table
                                ...
                                >
                                  <template slot="columns">
                                    ...
                                    <th><span v-if="lang=='english'">Business</span><span v-if="lang=='arabic'">عمل</span></th>
                                  ...
                                  >
                                    ...
                                    <th class="align-middle">
                                      <span class="mb-0 font-weight-400">{{ row.business_id.business_name_english }}</span>
                                    </th>
                                    <th class="align-middle">
                                      <base-button
                                        type="success"
                        ...
                        <div class="mb-3 d-none" id="plancontract">
                          ...
                          <div class="col-md-12 col-3 px-0 mt-3">
                            ...
                            <router-link to="/plans" class="btn btn-block btn-success"><span v-if="lang=='english'">View Saved Plans</span><span v-if="lang=='arabic'">View saved price files</span> <i class="fa fa-chevron-right"></i></router-link>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <form @submit.prevent> 
            <div class="row">
              <div class="col-lg-12 mb-3">
                <label class="form-control-label" v-if="lang=='english'">Business</label>
                <label class="form-control-label" v-if="lang=='arabic'">عمل</label>
                <base-dropdown tag="li" class="nav-item ml-lg-4 ml-2">
                  <a slot="title" class="btn btn-neutral btn-icon" data-toggle="dropdown" role="button">
                    <span slot="title" class="nav-link-inner--text">{{ this.selected_business.business_name ? this.selected_business.business_name : "Select Business" }}</span>
                  </a>
                  <div 
                    class="dropdown-item cursor-pointer" 
                    v-for="(item, index) in selectBusinessList" 
                    :key="index" 
                    
                  >
                    <base-checkbox class="" type="checkbox" :id="item._id" v-model="item.checkvalue" :onClick="checkBusiness">
                      {{item.business_name_english}}
                    </base-checkbox>
                  </div>
                </base-dropdown>
              </div>
      ...
      <modal :show.sync="openRequestLimitModal" modalClasses="modal-lg">
        <h6 class="modal-title" id="modal-title-default">
          <span v-if="lang=='english'">Your request count reached to limit. Please contact <a href="mailto: contact@nabnee.com">contact@nabnee.com</a></span>
...
import { link } from "fs";
    ...
    startplaning(){
      ...
      if (localStorage.getItem('accessToken')) {
        if (this.project_name) {
            axios.post(process.env.VUE_APP_API_ENDPOINT+'create_projectplan',{projectplan_name :this.project_name,projectplan_name_ar:"",created_by:localStorage.getItem('_id'),status:"approved"},this.auth)
          .then(response => { 
            debugger
            this.projectplan=response.data.details;
            
            localStorage.setItem('projectplan',response.data.details.projectplan_id)
          })
  ...
  async hadleimage(event) {
    ...
    openstartquote(row,id){
      ...
      this.businessData.map((item,index)=>{
        item.deals_in.map((dealItem,number)=>{
          if(dealItem==row.item_name)
          {
            this.selectBusinessList.push(item)
          }
        })
      })
      this.startquote=true;
    },
    onBusinessSelected(business_id, business_name) {
    ...
    async sendquery(){
      ...
      _this.auth)
        .then(response => {
          ...
          {
            if(_this.limitNumber > response.data.details.request_limit-response.data.details.request_count)
            {
              reachedToLimit = true
            }
          }
      ...
      if (reachedToLimit) {
        ...
        axios.post(process.env.VUE_APP_API_ENDPOINT+'create_quote', {
          ...
          debugger
            localStorage.setItem('send_query_state','true');
            axios.post(`${process.env.VUE_APP_API_ENDPOINT}increase_request_count`, {
                data: {
                  user_id: localStorage.getItem('user_id'),
                  amount: _this.businessId.length
                }
              },
              _this.auth);
          for(let i = 0; i < response1.data.details.length; i++)
          {
            _this.quotes.push(response1.data.details[i]._id);
          }
      ...
      _this.auth)
        .then(response => {
          ...
          else
          {
            ...
            _this.businessId.push(e.target.id)
            }
          }
        });
        if (reachedToLimit) {
        _this.openRequestLimitModal = true;
    ...
    beforeRouteLeave(to, from, next) {
      debugger;
      if(localStorage.getItem('projectplan'))
      { 
        if(localStorage.getItem('send_query_state')!=='true')
        {
          axios.post(process.env.VUE_APP_API_ENDPOINT+'delete_projectplan',{ 'projectplan_id':localStorage.getItem('projectplan')})
              .then(response => {
                localStorage.removeItem('projectplan');
        ...
        {
          localStorage.setItem('send_query_state','false');
        } 
      }
      next();
